...
if(NOT SANITIZER_MODE STREQUAL "None")
  ...
  # Sanitizer instrumentation applies to both C and C++; ASM_NASM is
  # intentionally excluded — NASM has no instrumentation, the runtime is
  # pulled in by the C/C++ link path.
  list(APPEND SAN_APPLIED_FLAGS
    # --- ASAN (Address Sanitizer) ---
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Address>,$<COMPILE_LANG_AND_ID:CXX,GNU,Clang>>:${ASAN_FLAGS}>
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Address>,$<COMPILE_LANG_AND_ID:C,GNU,Clang>>:${ASAN_FLAGS}>
    ...
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Thread>,$<COMPILE_LANG_AND_ID:C,GNU,Clang>>:${TSAN_FLAGS}>

    # --- MSAN (Memory Sanitizer) ---
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Memory>,$<COMPILE_LANG_AND_ID:CXX,GNU,Clang>>:${MSAN_FLAGS}>
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Memory>,$<COMPILE_LANG_AND_ID:C,GNU,Clang>>:${MSAN_FLAGS}>

    # --- UBSAN (Undefined Behavior Sanitizer) ---
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Undefined>,$<COMPILE_LANG_AND_ID:CXX,GNU,Clang>>:${UBSAN_FLAGS}>
    $<$<AND:$<STREQUAL:${SANITIZER_MODE},Undefined>,$<COMPILE_LANG_AND_ID:C,GNU,Clang>>:${UBSAN_FLAGS}>
  )
